  ...
  <div class="admin-layout">
    <aside class="admin-sidebar">
      ...
      <div class="admin-badge">
        <span>{{ authStore.adminProfile?.roleCode || 'operator' }}</span>
        <strong>{{ authStore.adminProfile?.adminName || '管理员' }}</strong>
      </div>
      <nav>
...
.admin-badge { padding: 18px; border-radius: 18px; background: rgba(255,255,255,0.08); }
.admin-badge span { display: inline-block; margin-bottom: 8px; text-transform: uppercase; font-size: 12px; opacity: 0.75; }
nav { display: grid; gap: 8px; }
nav a { padding: 12px 14px; border-radius: 12px; color: rgba(255,255,255,0.7); }
nav a.router-link-active { background: rgba(255,255,255,0.16); color: #fff; }
.admin-main { padding: 24px; }
.admin-topbar { display: flex; justify-content: space-between; align-items: center; padding: 22px; margin-bottom: 22px; }
.admin-topbar h1 { margin: 8px 0 0; font-family: var(--font-display); }
