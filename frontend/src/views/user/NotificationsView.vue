<template>
  <section class="glass-card panel">
    ...
    <el-radio-group v-model="readStatus" @change="fetchNotifications">
    ...
    <div class="notification-list">
      <article v-for="item in records" :key="item.notificationId" class="notice-row glass-card">
        ...
        <div class="notice-actions">
          ...
          <el-button size="small" @click="readOne(item.notificationId)">标记已读</el-button>
        </div>
      </article>
    </div>
  </section>
...
async function fetchNotifications() {
  const data = await userApi.getNotifications({
    page: 1,
    size: 50,
    readStatus: readStatus.value === 'all' ? undefined : readStatus.value
  });
