<template>
  <div class="page-shell register-page">
    <section class="register-card glass-card">
      <SectionHeading
        title="提交注册申请"
        description="先上传学生证，再填写身份信息。审核通过后才可登录系统。"
        tag="Campus Verification"
      />
      ...
      <div v-if="step === 0" class="step-panel">
        ...
        <div v-if="previewUrl" class="preview-card">
          ...
          <div class="preview-meta">
            ...
            ><small>{{ fileSizeText }}</small>
          </div>
        </div>
        <div class="upload-actions">
          <el-button @click="resetUpload">重新选择</el-button>
          <el-button type="primary" :loading="uploading" :disabled="!selectedFile" @click="uploadStudentCard"
            >上传并继续</el-button
          >
        </div>
      </div>
        ...
        <el-form ref="formRef" :model="form" :rules="rules" label-width="96px" class="register-form">
          <div class="form-grid">
            <el-form-item label="学号" prop="studentNo"
              ><el-input v-model="form.studentNo" maxlength="20"
            /></el-form-item>
            ...
            <el-form-item label="性别" prop="gender"
              ><el-select v-model="form.gender"
                ><el-option label="男" value="male" /><el-option label="女" value="female" /></el-select
            ></el-form-item>
            ...
            <el-form-item label="学院" prop="collegeName"
              ><el-input v-model="form.collegeName" maxlength="50"
            /></el-form-item>
            <el-form-item label="专业" prop="majorName"
              ><el-input v-model="form.majorName" maxlength="50"
            /></el-form-item>
            <el-form-item label="班级" prop="className"
              ><el-input v-model="form.className" maxlength="50"
            /></el-form-item>
          </div>
        </el-form>
        <div class="form-actions">
          <el-button @click="step = 0">返回</el-button
          ><el-button type="primary" :loading="submitting" @click="submitRegistration">提交申请</el-button>
        </div>
      </div>
      <div v-else class="step-panel done-panel">
        <el-result
          icon="success"
          title="申请已提交"
          sub-title="注册申请已进入管理员审核流程，审核结果会通过站内通知或邮件反馈。"
          ><template #extra><el-button type="primary" @click="goResult">查看提交结果页</el-button></template></el-result
        >
      </div>
    </section>
  </div>
...
async function uploadStudentCard() {
  ...
  try {
    const result = await publicApi.uploadStudentCard(selectedFile.value);
...
async function submitRegistration() {
  ...
  try {
    const result = await publicApi.submitRegistration({
      ...
      studentCardFileId: uploadedFileId.value
    });
...
function goResult() {
  router.push({ path: '/register-result', query: { applicationNo: applicationNo.value } });
}
</script>
...
.register-banner {
  ...
  padding: 18px 20px;
  border-radius: 22px;
  background: linear-gradient(135deg, rgba(25, 63, 58, 0.08), rgba(220, 127, 57, 0.1));
}
.register-banner strong {
  display: block;
  margin-bottom: 8px;
  font-size: 20px;
}
