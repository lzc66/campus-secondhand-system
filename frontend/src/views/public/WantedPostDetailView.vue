  ...
  <div class="page-shell detail-page" v-if="detail">
    <section class="glass-card detail-card">
      ...
      <el-descriptions :column="1" border>
        <el-descriptions-item label="期望价格区间"
          >{{ formatPrice(detail.expectedPriceMin) }} - {{ formatPrice(detail.expectedPriceMax) }}</el-descriptions-item
        >
...
onMounted(async () => {
  detail.value = await publicApi.getWantedPostDetail(Number(route.params.id));
});
</script>
