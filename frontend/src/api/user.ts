...
export const userApi = {
  ...
  uploadAvatar: (file: File) => {
    ...
    return apiPost<any>('/api/v1/user/profile/avatar', formData, {
  ...
  updateItem: (itemId: number, payload: Record<string, unknown>) =>
    apiPut<any>(`/api/v1/user/items/${itemId}`, payload),
  deleteItem: (itemId: number) => apiDelete<any>(`/api/v1/user/items/${itemId}`),
  createComment: (itemId: number, payload: Record<string, unknown>) =>
    apiPost<any>(`/api/v1/user/items/${itemId}/comments`, payload),
  replyComment: (commentId: number, payload: Record<string, unknown>) =>
    apiPost<any>(`/api/v1/user/comments/${commentId}/reply`, payload),
  getReceivedComments: (params?: Record<string, unknown>) => apiGet<any>('/api/v1/user/comments/received', { params }),
  getWantedPosts: (params?: Record<string, unknown>) =>
    apiGet<PageResponse<WantedPost>>('/api/v1/user/wanted-posts', { params }),
  getWantedPostDetail: (wantedPostId: number) => apiGet<WantedPost>(`/api/v1/user/wanted-posts/${wantedPostId}`),
  saveWantedPost: (payload: Record<string, unknown>, wantedPostId?: number) =>
    wantedPostId
      ? apiPut<any>(`/api/v1/user/wanted-posts/${wantedPostId}`, payload)
      : apiPost<any>('/api/v1/user/wanted-posts', payload),
  ...
  confirmOrder: (orderId: number, payload: Record<string, unknown>) =>
    apiPost<any>(`/api/v1/user/orders/${orderId}/confirm`, payload),
  deliverOrder: (orderId: number, payload: Record<string, unknown>) =>
    apiPost<any>(`/api/v1/user/orders/${orderId}/deliver`, payload),
  completeOrder: (orderId: number, payload: Record<string, unknown>) =>
    apiPost<any>(`/api/v1/user/orders/${orderId}/complete`, payload),
  cancelOrder: (orderId: number, payload: Record<string, unknown>) =>
    apiPost<any>(`/api/v1/user/orders/${orderId}/cancel`, payload),
  getNotifications: (params?: Record<string, unknown>) =>
    apiGet<PageResponse<NotificationItem>>('/api/v1/user/notifications', { params }),
  ...
  getRecommendations: (params?: Record<string, unknown>) =>
    apiGet<PageResponse<RecommendationItem>>('/api/v1/user/recommendations', { params }),
  refreshRecommendations: (limit = 12) => apiPost<any>(`/api/v1/user/recommendations/refresh?limit=${limit}`),
  clickRecommendation: (recommendationId: number) =>
    apiPost<any>(`/api/v1/user/recommendations/${recommendationId}/click`)
};
