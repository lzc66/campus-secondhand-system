...
class SystemInitServiceTest {
    ...
    @Test
    void shouldNotResetChangedPasswordOrReactivateDisabledAdmin() {
        when(itemCategoryMapper.selectByCategoryCode(anyString())).thenAnswer(invocation -> {
            String code = invocation.getArgument(0);
            return switch (code) {
                case "digital_devices" -> category(code, "数码设备", 10);
        ...
        // 管理员已改过密码且已被停用:bootstrap 必须原样保留,不得回写默认哈希或重新激活
        when(adminMapper.selectByAdminNo("admin1001")).thenReturn(Admin.builder()
                .adminId(1001L)
                .adminNo("admin1001")
                .passwordHash("$2b$10$changedPasswordHashChangedPasswordHashChanged")
                .adminName("Renamed Admin")
                .email("renamed@campus.local")
                .roleCode(AdminRoleCode.SUPER_ADMIN)
